---
name: seo-audyt
description: Robi audyt SEO strony jacekfiedziukiewicz.pl (Next.js App Router) przed publikacją i opcjonalnie na żywej domenie. Sprawdza metadane per podstrona, kanoniczne, OpenGraph, jeden H1, alty, sitemap/robots, JSON-LD, wagę obrazów i linkowanie wewnętrzne. Uruchom, gdy użytkownik mówi "audyt seo", "sprawdź seo", "czy strona jest pod seo", "/seo-audyt". Zwraca raport PASS/FAIL/TODO, nie zmyśla wyników.
---

# Audyt SEO — strona JACA AI
...
- Czy jest `export const metadata` (lub `generateMetadata` dla `[slug]`)? Strony dynamiczne
  (`blog/[slug]`, `case-study/[slug]`) powinny mieć `generateMetadata` z tytułem z treści.
...
- `openGraph` z `images` (bazowo `/images/og-image.avif`; mocne podstrony mogą mieć własny OG).
- W `app/layout.tsx`: `metadataBase`, `lang="pl"`, bazowy OG `locale: "pl_PL"`.

## Krok 2 — struktura treści

- Dokładnie **jeden `<h1>`** na podstronie (w portach Webflow sprawdź `heading-style-h1` / `<h1>`
  w `app/_webflow/<nazwa>.html`; w React — w `page.tsx`).
...
- `app/sitemap.ts` — jeśli brak, **zaproponuj** (natywny generator Next, wylistuj statyczne routes
  + dynamiczne slugi z `getBlogSlugs()` / `getCaseSlugs()` z `lib/sanity.ts`).
...
Zaproponuj `Person` / `ProfessionalService` na stronie głównej i `Service` na stronach
ofertowych (`/wdrozenie`). `Article` dla wpisów blogowych. To propozycja do wdrożenia, nie
...
- Nowy kod powinien używać `next/image` zamiast surowego `<img>` (porty Webflow zostają jak są).
